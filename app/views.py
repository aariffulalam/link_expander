...
from SESService import SnsEmailService  # Updated to use SnsEmailService
...
class URLViews:
    ...
    async def handle_url_expansion(self, url, toReturn):
        try:
            ...
            if response is None or not response.get("expanded", False):
                toReturn["expanded"] = False
                toReturn["error_message"] = response.get("error_message", "Error in LinkExpander")
    ...
    def send_success_email(self, url, toReturn):
        try:
            ...
            topic_arn = "arn:aws:sns:us-east-1:123456789012:YourTopicName"  # Replace with your actual topic ARN
            subject = "URL Expansion Success Notification"
            message_id = self.email_service.send_email(topic_arn, subject, email_params)
            if message_id:
    ...
    async def check_flipkart_and_unknown_patterns(self, url, toReturn):
        ...
        """
        flipkart_patterns = ["flipkart.com/s/", "fkrt.it", "fkrt.cc", "fkrt.to", "fkrt.co"]

        try:
            # Check if the URL matches any Flipkart-specific patterns
            if any(pattern in toReturn["expanded_url"] for pattern in flipkart_patterns) or toReturn['expanded'] is False:
                logger.info(f"Flipkart pattern detected in URL: {toReturn['expanded_url']}")
                ...
                if responseOfPWScrapper and len(responseOfPWScrapper) > 0:
                    ...
                    logger.info(f"PWScrapper expanded Flipkart URL successfully: {expanded}")
                else:
                    toReturn["expanded"] = False
                ...
                logger.warning(f"Unknown pattern detected in URL: {toReturn['expanded_url']}")
                toReturn["expanded"] = False
